...
import { Upload, Image as ImageIcon, Wand2, Download, AlertCircle, RefreshCw, ChevronRight, Sparkles, Type, Paperclip, X, Copy, Check, Code, Palette, Eye, Plus, Replace } from 'lucide-react';
import Header from './components/Header';
import LoadingOverlay from './components/LoadingOverlay';
import { editImageWithGemini, generateMultimodalImage, generateSvgWithGemini } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const [referenceImages, setReferenceImages] = useState<ImageAsset[]>([]); // Changed to array
  ...
  const fileInputRef = useRef<HTMLInputElement>(null); // For Edit Tab main upload
  const attachmentInputRef = useRef<HTMLInputElement>(null); // For Generate Tab attachment

  // Extract SVG if present in generated text
  const svgContent = useMemo(() => generatedText ? extractSvg(generatedText) : null, [generatedText]);

  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>, isAttachment: boolean = false) => {
    ...
    reader.onloadend = () => {
      ...
      if (match) {
        ...
        if (isAttachment) {
          ...
          // Only reset results if we are newly entering the flow, otherwise just swapping source
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (outputMode === 'svg') {
        // SVG/Code Mode
        const imagesToUse = activeTab === 'edit' && originalImage 
          ? [originalImage] 
          : referenceImages;
        ...
        if (activeTab === 'edit' && originalImage) {
          ...
          // Generate tab (handles 0 to 3 reference images)
      ...
      if (result.imageUrl) {
        ...
        setErrorMsg("The model returned text but no image. Try a more specific visual prompt.");
  ...
  // Helper to open upload for Replace action
  const triggerReplaceOriginal = () => {
    if (fileInputRef.current) fileInputRef.current.click();
  };

  const showWorkspace = (activeTab === 'edit' && originalImage) || (activeTab === 'generate' && (appState !== AppState.IDLE || generatedImage || generatedText || referenceImages.length > 0 || prompt.length > 0));
  const suggestedPrompts = activeTab === 'edit' ? SUGGESTED_EDIT_PROMPTS : SUGGESTED_GEN_PROMPTS;
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 font-sans selection:bg-brand-500/30">
        ...
        {!showWorkspace && (
          <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-8 animate-in fade-in zoom-in duration-500">
            ...
            <div className="p-1 bg-slate-900 rounded-full border border-slate-800 flex relative">
               <button 
                onClick={() => { setActiveTab('edit'); setErrorMsg(null); }}
                className={`relative z-10 px-6 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeTab === 'edit' ? 'text-white bg-slate-800 shadow-sm border border-slate-700' : 'text-slate-500 hover:text-slate-300'}`}
               >
                 Edit Image
               </button>
               <button 
                onClick={() => { setActiveTab('generate'); setErrorMsg(null); }}
                className={`relative z-10 px-6 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeTab === 'generate' ? 'text-white bg-slate-800 shadow-sm border border-slate-700' : 'text-slate-500 hover:text-slate-300'}`}
               >
                 Generate Image
               </button>
            </div>
              ...
              >
                ...
                <div className="relative px-12 py-10 bg-slate-900 rounded-xl border border-slate-800 hover:border-brand-500/50 transition-colors flex flex-col items-center gap-4">
                  ...
                  <div className="text-center">
                    <p className="text-lg font-semibold text-white group-hover:text-brand-200">Click to Upload Image</p>
              ...
              <div className="w-full max-w-xl space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="relative group">
                  ...
                  <div className="relative bg-slate-900 rounded-xl p-2 border border-slate-800 hover:border-brand-500/50 transition-colors">
                    ...
                    {/* Render up to 3 reference images */}
                    {referenceImages.length > 0 && (
                      <div className="mx-4 mt-2 mb-1 flex gap-2 flex-wrap">
                        {referenceImages.map((img) => (
                           <div key={img.id} className="relative rounded-lg overflow-hidden border border-slate-700 group/preview w-20 h-20">
                             <img src={img.url} alt="Reference" className="w-full h-full object-cover opacity-80" />
                             <button 
                    ...
                    <textarea
                      ...
                      placeholder={referenceImages.length > 0 ? "Describe how to use these images..." : "Describe the image you want to generate..."}
                       ...
                       <div className="flex items-center gap-2">
                         <div className="flex bg-slate-950 rounded-lg p-0.5 border border-slate-800">
                            <button
                                ...
                                className={`px-2 py-1 rounded-md text-xs flex items-center gap-1 transition-colors ${outputMode === 'image' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}
                            ...
                            <button
                                ...
                                className={`px-2 py-1 rounded-md text-xs flex items-center gap-1 transition-colors ${outputMode === 'svg' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}
              ...
              {activeTab === 'generate' && (
                <div className="flex items-center justify-between">
                   <h3 className="text-lg font-semibold text-white flex items-center gap-2">
                    ...
                    {referenceImages.length > 0 ? `Text to Image (${referenceImages.length} references)` : "Text to Image"}
                  </h3>
                  <button 
                    onClick={resetApp}
              ...
              <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl">
                <div className="flex justify-between items-center mb-2">
                   <label className="block text-sm font-medium text-slate-300">
                      {activeTab === 'edit' || referenceImages.length > 0 ? "Context & Prompt" : "Refine your prompt"}
                   </label>
                   
                   <div className="flex bg-slate-950 rounded-lg p-0.5 border border-slate-700">
                        <button
                            onClick={() => setOutputMode('image')}
                            className={`px-3 py-1 rounded-md text-xs flex items-center gap-1.5 transition-colors ${outputMode === 'image' ? 'bg-slate-800 text-white shadow-sm font-medium' : 'text-slate-500 hover:text-slate-300'}`}
                        ...
                        <button
                            onClick={() => setOutputMode('svg')}
                            className={`px-3 py-1 rounded-md text-xs flex items-center gap-1.5 transition-colors ${outputMode === 'svg' ? 'bg-slate-800 text-white shadow-sm font-medium' : 'text-slate-500 hover:text-slate-300'}`}
                            title="Generate SVG Code"
                        >
                ...
                <div className="relative">
                   {/* Reference Images Grid inside the Prompt Box area */}
                   {activeTab === 'generate' && referenceImages.length > 0 && (
                      <div className="flex gap-2 p-2 absolute top-0 left-0 right-0 z-10 bg-gradient-to-b from-slate-900/90 to-transparent">
                         {referenceImages.map((img) => (
                            <div key={img.id} className="relative rounded-md overflow-hidden border border-slate-700 shadow-lg bg-slate-950 w-16 h-16 flex-shrink-0">
                              <img src={img.url} alt="Reference" className="w-full h-full object-cover opacity-80" />
                              <button 
                                onClick={(e) => removeReferenceImage(img.id, e)}
                  ...
                  <textarea
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    placeholder={outputMode === 'svg' ? "E.g., 'Create a minimalist SVG logo for Road Runner...'" : "Describe your image..."}
                    className={`w-full bg-slate-950 border border-slate-700 rounded-xl p-4 text-slate-100 placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-brand-500/50 resize-none h-40 transition-shadow ${referenceImages.length > 0 && activeTab === 'generate' ? 'pt-20' : ''}`}
                  ...
                  <div className="absolute bottom-3 left-3">
                     {activeTab === 'generate' && referenceImages.length === 0 && (
                        <button 
                           onClick={() => attachmentInputRef.current?.click()}
                           className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-brand-300 transition-colors"
                           title="Attach image"
                         >
                           <Paperclip className="w-4 h-4" />
                           <span className="hidden sm:inline">Attach (0/3)</span>
                         </button>
                     )}
                     {activeTab === 'generate' && referenceImages.length > 0 && (
                        <span className="text-xs text-slate-500">{referenceImages.length}/3 images attached</span>
                     )}
                  </div>
                  ...
                  <div className="absolute bottom-3 right-3">
                     <button
                      ...
                      `}
                    >
                      {appState === AppState.PROCESSING ? 'Processing...' : 'Generate'}
                      {appState !== AppState.PROCESSING && <Wand2 className="w-4 h-4" />}
                    </button>
                  </div>
                </div>
                    ...
                    <div className="w-full h-full flex flex-col">
                       {svgContent && (
                          <div className="flex border-b border-slate-800 bg-slate-900/50">
                             <button onClick={() => setViewMode('preview')} className={`px-4 py-2 text-xs font-medium transition-colors flex items-center gap-1.5 ${viewMode === 'preview' ? 'text-brand-400 border-b-2 border-brand-500 bg-slate-800/50' : 'text-slate-500 hover:text-slate-300'}`}>
                               <Eye className="w-3.5 h-3.5" /> Preview
                             </button>
                             <button onClick={() => setViewMode('code')} className={`px-4 py-2 text-xs font-medium transition-colors flex items-center gap-1.5 ${viewMode === 'code' ? 'text-brand-400 border-b-2 border-brand-500 bg-slate-800/50' : 'text-slate-500 hover:text-slate-300'}`}>
                       ...
                       <div className="flex-1 overflow-auto p-4 custom-scrollbar relative">
                          {svgContent && viewMode === 'preview' ? (
                             <div className="w-full h-full flex items-center justify-center min-h-[300px]" dangerouslySetInnerHTML={{ __html: svgContent }} />
                          ) : (
      ...
      <footer className="w-full py-6 mt-12 border-t border-slate-900 text-center">
        <p className="text-slate-600 text-sm">
          &copy; {new Date().getFullYear()} Road Runner Studio. Powered by Google Gemini.
        </p>
      </footer>

      {/* Inputs moved here, always rendered */}
      <input 
        type="file" 
        ref={fileInputRef} 
